...
def retrieveURLWithEachUserAgent(url, userAgents=userAgents, timeout=2):
	...
	try: # Detect an invalid URL early
		...
		printError("Invalid URL:", url, "Exception:", err)
		return []
	...
	for uA in userAgents:
		try:
			...
			printError("HTTP error retrieving URL:", url, "Agent:", uA,
			...
			if ((type(err.reason) is str) and
				...
				printError("Invalid URL:", url, "Reason:", err.reason)
			else:
				printError("URL error retrieving URL:", url, "Agent:", uA,
						"Reason:", err.reason)
			return [] # URLErrors are regardless of agent, so don't try other agents
		except Exception as err:
			printError("Unknown error retrieving URL:", url, "Agent:", uA,
					"Exception:", err)
		else:

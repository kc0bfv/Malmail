...
class RetrievedData(EqualityWithToTuple):
	...
	def extractURLs(self): return self.data.extractURLs()
	...
	def extractURLs(self):
		class URLExtractor(htp.HTMLParser):
			...
			def __init__(self, *args, **kwargs):
				super().__init__(strict=False, *args, **kwargs)
				self.urlList = []
...
class MultipartEmailContent(Content):
	...
	def body(self):
		return "\n".join(part.body() for part in self.body)
...
class ContentHandlersTester(unittest.TestCase):
	...
	def test_EmailData_extractURLs(self):
		...
		for (urlList, contents) in testEmails:
			# Check the equality of these as sets, because order doesn't matter
			self.assertEqual(set(urlList), set(EmailData(contents).extractURLs()))
